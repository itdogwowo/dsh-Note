---
name: git-push
description: '一次同意完成提交與推送。當使用者說「幫我推」「推上去」「push 上去」「commit 然後推」「提交」「發佈」時載入。流程：偵測 git → 挑選檔案 → 擬 commit 訊息 → 顯示完整計畫並取得**一次**同意 → 同意後 commit 與 push 一次做完。⚠️ 同意只對那一次有效；force push 必須另外同意；沒回答不等於同意。'
...
| 選擇 | 做什麼 |
|---|---|
| ① 同意 | **commit ＋ push 一次做完**，不要再問第二次 |
| ② 只提交 | commit 完就停，**不要再問一次要不要推** |
| ③ 要改 | 改完**重新顯示同意卡**，再問一次 |
